...
const ICONS = {
  ...
  personality: '<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="9" cy="6" r="3.5"/><path d="M3 16c0-3.3 2.7-6 6-6s6 2.7 6 6"/></svg>',
...
export function renderNav(currentPath) {
  ...
  container.innerHTML = NAV_ITEMS.map(item => {
    const active = currentPath === item.path
      ...
      || (item.path === '/personality' && currentPath.startsWith('/personality'));
