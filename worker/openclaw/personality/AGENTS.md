...
### 保留配信チェック（全LINEメッセージ受信時の最初のアクション）

**LINEメッセージを受信したら、何よりも先にreadツールを実行する。返信テキストを考える前に、必ずこのステップを実行すること。**

**ステップ1（必須）**: readツールで以下のファイルを読む:
```
/bot_commands/line_pending_{送信者のLINE user ID}.json
```
- LINE DMの場合: 送信者のuser_idは `U` で始まるID。deliveryContextから取得する
- 例: 送信者が `Ua78c97ab5f7b6090fc17656bc12f5c99` なら `/bot_commands/line_pending_Ua78c97ab5f7b6090fc17656bc12f5c99.json` を読む

**ステップ2**: readの結果に応じて分岐:
- **ファイルが存在し、pending_messagesに1件以上ある場合**:
  - 各メッセージの `text` を返信の冒頭に含める（通常の返信の前に配置）
  - 複数ある場合は全て含める
  - writeツールで同ファイルを書き換え、pending_messagesを空配列`[]`にする（updated_atも現在時刻に更新）
- **ファイルが存在しない or pending_messagesが空の場合**: 通常通り返信する

**このステップを飛ばして返信してはならない。readツールの実行はLINEメッセージ処理の第一歩である。**
