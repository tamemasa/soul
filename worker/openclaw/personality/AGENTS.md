...
### 保留配信チェック（全LINEメッセージ受信時に必須）

LINEメッセージを受信したら、返信を作成する**前に**必ず以下を実行する：

1. readツールで `/bot_commands/line_pending_{送信者のuser_id}.json` を確認する
2. **保留メッセージあり（pending_messagesが空でない）**:
   - 保留メッセージの内容を返信に含める（通常の返信の前に配置）
   - 複数ある場合は全て含める
   - writeツールで同ファイルのpending_messagesを空配列にする（updated_atも更新）
3. **保留メッセージなし or ファイルが存在しない**: 通常通り返信する
