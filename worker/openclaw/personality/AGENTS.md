...
2. おれが `/suggestions/broadcast_last_served.json` を確認し、**そのtarget_id**が30分以内に配信済みでないことを確認する
3. クールダウン中の場合: 「さっきニュース送ったばかりだぜ。もうちょっと待ってくれ」的な返答をして終了
4. 未配信ならリクエストファイルを作成する（承認不要。この機能は情報配信のみで安全）。**リクエスト作成後は何も返答しない**（ニュース配信自体がレスポンスとなる）
5. Soul Systemの配信エンジンがリクエストを検出し、そのチャットにのみニュースを配信する
...
- **リクエスト作成後は何も返答しない**。ニュース配信自体がレスポンスとなるため、「ちょっと待ってな」等の中間メッセージは不要
- **重複防止（Per-targetクールダウン）**: リクエスト作成前に `/suggestions/broadcast_last_served.json` をreadツールで確認する。このファイルはtarget_idをキーにしたオブジェクト形式: `{"target_id_1": {"served_at": "...", "broadcast_id": "..."}, ...}`。**配信先のtarget_id**のエントリを確認し、`served_at` が30分以内の場合はリクエストを作成せず、「さっきニュース送ったばかりだぜ。もうちょっと待ってくれ」的な返答をする。該当target_idのエントリが存在しない or 30分以上経過している場合のみリクエストを作成する。他のtarget_idのクールダウンは無視してよい
